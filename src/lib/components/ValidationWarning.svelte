...
<div
    class="{hasErrors
        ...
        : 'bg-amber-50 border-amber-300'} border rounded-lg p-4 flex items-start gap-3 shadow-sm"
>
    <div class="flex-1">
        <!-- Header -->
        <div class="flex items-center justify-between mb-2">
            <h4
                class="text-base font-semibold flex items-center gap-1.5 {hasErrors
                    ? 'text-red-900'
                    : 'text-amber-900'}"
            >
                <svg
        ...
        <div class="space-y-2 mb-3">
            {#each Array.from(issuesByType) as [type, issues]}
                <div
                    class="border {hasErrors && issues[0].severity === 'error'
                        ? 'border-red-200 bg-red-50'
                        : 'border-amber-200 bg-amber-50'} rounded-md overflow-hidden"
                >
                    <button
                        type="button"
                    ...
                    >
                        <div class="flex items-center gap-2">
                            ...
                            {:else}
                                <span
                                    class="text-xs font-semibold text-amber-700 bg-amber-200 px-2 py-0.5 rounded"
                                    >WARNING</span
                                >
                            {/if}
                            <span
                            ...
                            <span
                                class="text-xs {issues[0].severity === 'error'
                                    ? 'text-red-600'
                                    : 'text-amber-600'}"
                            >
                                ({issues.length}
                                ...
                                <div class="text-xs space-y-1">
                                    ...
                                    <code
                                        class="{issue.severity === 'error'
                                            ...
                                            : 'bg-amber-100 text-amber-800'} block px-2 py-1 rounded text-xs overflow-x-auto"
                                    >
                                        {issue.raw}
                                    </code>
                                    {#if issue.suggestion}
                                        <p
                                            class="{issue.severity === 'error'
                                                ? 'text-red-600'
                                                : 'text-amber-600'} italic flex items-start gap-0.5"
                                        >
                                            <svg
                                                xmlns="http://www.w3.org/2000/svg"
                                                viewBox="0 0 24 24"
                                                class="shrink-0 mt-px size-3.5"
                                            >
                                                <path
                                                    fill="currentColor"
        ...
        {#if validationResult.hasMultiTimestamps}
            <div
                class="text-xs {hasErrors
                    ? 'text-red-700 bg-red-100/50'
                    : 'text-amber-700 bg-amber-100/50'} rounded p-2 mb-3"
            >
        ...
        <!-- Explanation for ELRC -->
        {#if validationResult.hasELRC}
            <div class="text-xs text-red-700 bg-red-100/50 rounded p-2 mb-3">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            {#if showStripELRCButton && onStripELRC}
                <button
                    type="button"
                    onclick={onStripELRC}
                    class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium
